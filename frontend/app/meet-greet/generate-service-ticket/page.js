    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (!response.ok) {
                ...
                if (cardHolderDetails.meet_limit > 0) {
                    await fetch(`http://localhost:5000/card-holders/${watchCardHolderId}`, {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            ...selectedCardHolder,
                            meet_limit: cardHolderDetails.meet_limit - 1
                        })
                    });
                }
                        ...
                        <Card sx={{ borderLeft: "5px solid #667eea" }}>
                            ...
                            <CardContent>
                                <Grid container spacing={2}>
                                    ...
                                    {/* Alert for 0 limit */}
                                    {selectedCardHolder && cardHolderDetails.meet_limit === 0 && (
                                        <Grid item size={12}>
                                            <div className="mt-2 p-5 rounded-xl border-2 border-red-500 bg-red-50 shadow-[0_4px_20px_rgba(239,68,68,0.3)] relative overflow-hidden flex items-start gap-4">
                                                ...
                                                <div className="relative z-10">
                                                    ...
                                                    <p className="text-red-800 font-medium">
                                                        This card holder has <strong className="text-red-900 border-b-2 border-red-900">0</strong> meet and greet limits remaining. The system is locked and free services cannot be booked.
                                                    </p>
                                                </div>
                                            </div>
                                        </Grid>
                    ...
                    <Grid item xs={12}>
                        <Box sx={{ display: "flex", gap: 2, justifyContent: "flex-end", flexWrap: "wrap" }}>
                            ...
                            <Button
                                type="submit"
                                variant="contained"
                                disabled={selectedCardHolder && cardHolderDetails.meet_limit === 0}
                                sx={{
                                    px: 4,
                                    textTransform: "none",
                                    fontWeight: 600,
                                    background: selectedCardHolder && cardHolderDetails.meet_limit === 0 ? "#ccc" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
                                    boxShadow: selectedCardHolder && cardHolderDetails.meet_limit === 0 ? "none" : "0 4px 12px rgba(102, 126, 234, 0.4)",
                                    "&:hover": {
                                        boxShadow: selectedCardHolder && cardHolderDetails.meet_limit === 0 ? "none" : "0 6px 16px rgba(102, 126, 234, 0.6)",
                                        background: selectedCardHolder && cardHolderDetails.meet_limit === 0 ? "#ccc" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
                                    },
                                }}
                            >
